...
# ─── Nhóm bảng theo spec ─────────────────────────────────────
EVENT_TABLES   = {"play_history", "payments", "likes", "follows", "ad_impressions"}
CATALOG_TABLES = {"users", "songs", "artists", "albums", "genres", "subscriptions", "ads"}
...
def load_last_run(reset: bool = False) -> dict:
    if reset or not LAST_RUN_FILE.exists():
        return {
            ...
            "next_extract_from": "2000-01-01T00:00:00"
        }
...
def write_log(conn, run_id: int, run_type: str, status: str,
    ...
    sql = """
        INSERT INTO etl_logs
            (run_id, run_type, status, extract_from, extract_to, total_rows, details, error_msg)
        VALUES
            (%(run_id)s, %(run_type)s, %(status)s, %(extract_from)s, %(extract_to)s,
             %(total_rows)s, %(details)s, %(error_msg)s)
    """
...
def write_bronze(table: str, rows: list, run_id: int, extract_from: str, extract_to: str):
    ...
    ts_str = datetime.now(timezone.utc).strftime("%Y_%m_%d_%H_%M_%S")
...
def run_extract(reset: bool = False):
    last_run     = load_last_run(reset=reset)
    extract_from = last_run["next_extract_from"]
    extract_to   = datetime.now(timezone.utc).isoformat()
    run_id       = last_run["last_success_run_id"] + 1
        ...
        if conn:
            try:
                write_log(conn, run_id, run_type, "error",
                          extract_from, extract_to, total_rows, results, str(e))
            except Exception:
                pass
        print(f"\n❌ Extract lỗi: {e}")
        raise
